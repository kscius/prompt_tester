...
- Gemini: al guardar API key, `credentials.json` (service account) solo se elimina después de persistir `provider-config.json` con éxito (antes un fallo de escritura podía borrar la SA de forma irreversible).
